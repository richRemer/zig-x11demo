...
pub const Display = struct {
    ...
    pub fn connect(this: *Display) !void {
        const parsed = parseDisplay(this.name);
...
test "protocol delimiters" {
    try std.testing.expectEqual(':', Protocol.unix.getDelimiter());
    try std.testing.expectEqual('/', Protocol.tcp.getDelimiter());
    try std.testing.expectEqual('/', Protocol.inet.getDelimiter());
    try std.testing.expectEqual('/', Protocol.inet6.getDelimiter());
}
...
test "initialize Display with 'unix' protocol" {
    var display = try Display.init("unix:");
    try std.testing.expectError(error.FileNotFound, display.connect());
}
